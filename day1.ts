...
function captcha(input: string) {
  let captcha = input.split('');
  let sum = 0;
  captcha.forEach((char, index) => {
    let valid: boolean;
...
function isValid(char1: string, char2: string) {
...
function captcha2(input: string) {
  ...
  captcha.forEach((char, index) => {
    let valid: boolean;
    if ((index + half) < captcha.length) { // not circular
      valid = isValid(char, captcha[index + half]);
    } else if ((index + half) >= captcha.length) { // circular
